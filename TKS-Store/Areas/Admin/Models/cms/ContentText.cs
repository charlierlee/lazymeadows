...
namespace TKS.Areas.Admin.Models.CMS {
	public class ContentText {
		...
		#region Constructor
		public ContentText() { }
		public ContentText(Guid ModuleID) {
			_ModuleID = ModuleID;
			Initialize();
		}
		public ContentText(ContentTextViewModel contentTextViewModel) {
			_ContentID = contentTextViewModel.ContentID;
			_ModuleID = contentTextViewModel.ModuleID;
			Initialize();
		}
		public ContentText(ContentTextDraftViewModel contentTextDraftViewModel) {
			_ContentID = contentTextDraftViewModel.ContentID;
			_ModuleID = contentTextDraftViewModel.ModuleID;
			Initialize();
		}
		#endregion
		...
		public List<ContentTextHistoryViewModel> History() {
			...
			using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString)) {
				...
				using (SqlCommand cmd = new SqlCommand(SQL, cn)) {
					...
					while (dr.Read()) {
						l.Add(new ContentTextHistoryViewModel() { ContentID = dr.GetGuid(0), InsertDate = dr.GetDateTime(1), ModuleID = _ModuleID, PageID = _PageID });
					}
					cmd.Connection.Close();
				}
			}
		...
		public void SaveChanges(ContentTextViewModel contentPageViewModel) {
			this.Contents = contentPageViewModel.Contents;
		}
		public void SaveChanges(ContentTextDraftViewModel contentPageDraftViewModel) {
			this.DraftContents = contentPageDraftViewModel.DraftContents;
			if (contentPageDraftViewModel.MakeLive) {
				MakeDraftLive();
			}
		}
		#endregion
	...
	public class ContentTextViewModel {
		#region Fields
		private Guid _ContentID = Guid.Empty;
		...
		#region Constructor
		public ContentTextViewModel() { }
		public ContentTextViewModel(ContentText contentText) {
			_Contents = contentText.Contents;
	...
	public class ContentTextDraftViewModel {
		#region Fields
		private Guid _ContentID = Guid.Empty;
		...
		#region Constructor
		public ContentTextDraftViewModel() { }
		public ContentTextDraftViewModel(ContentText contentText) {
			_ContentID = contentText.ContentID;
	...
	public class ContentTextSet {
		#region Fields
		private string _Locale = "en-US";
		#endregion

		#region Constructor
		public ContentTextSet(string Locale = "en-US") {
			_Locale = Locale;
		}
		#endregion

		#region Public Methods
		public List<ContentText> AllContentTextBlocks() {
			List<ContentText> l = new List<ContentText>();
			...
			using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString)) {
				...
				using (SqlCommand cmd = new SqlCommand(SQL, cn)) {
					...
					SqlDataReader dr = cmd.ExecuteReader();
					while (dr.Read()) {
						l.Add(new ContentText(dr.GetGuid(0)));
					}
					cmd.Connection.Close();
				}
			}
	...
	public class ContentTextHistoryViewModel {
